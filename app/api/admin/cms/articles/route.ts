...
export async function POST(request: Request) {
  ...
  try { await createArticle({ ...validation.data, body: sanitizeRichTextBody(validation.data.body) }); return Response.json({ message: 'Article created.', warnings: validation.warnings, workspace: await getContentWorkspace() }, { status: 201 }); } catch (error) { return adminRouteError(error); }
}
