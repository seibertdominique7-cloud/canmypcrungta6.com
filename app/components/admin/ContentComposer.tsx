  ...
  return (
    <section className="grid gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button className={smallButton} onClick={() => setProductPickerOpen(true)} type="button">Add Product</button>
      ...
      <div className={`grid gap-4 ${mode === 'split' ? 'xl:grid-cols-2' : ''}`}>
        {mode !== 'preview' ? <RichTextEditor body={body} onChange={onChange} ref={editorRef} /> : null}
        {mode !== 'write' ? (
          <DraftPreview
            body={body}
            excerpt={excerpt}
      ...
      {productPickerOpen ? (
        <ProductPicker
          onAdd={(product) => editorRef.current?.insertHtml(affiliateProductHtml(product))}
...
function ProductPicker({ products, selectedIds, search, setSearch, onAdd, onClose }: { products: Product[]; selectedIds: string[]; search: string; setSearch: (value: string) => void; onAdd: (product: Product) => void; onClose: () => void }) {
    ...
    <div aria-modal="true" className="fixed inset-0 z-[90] overflow-y-auto bg-black/80 p-4 backdrop-blur-sm" role="dialog">
      <div className="mx-auto my-10 max-w-3xl rounded-3xl border border-white/15 bg-[#0d111d] p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-black uppercase tracking-wide text-violet-300">Affiliate catalog</p><h2 className="mt-1 text-xl font-black">Add Product</h2><p className="mt-1 text-sm text-slate-400">Add any enabled product. The published card stays connected to its admin-managed data.</p></div>
            ...
            return (
              <article className="flex min-h-28 gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3" key={product.id}>
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  ...
                  <button className={`${selected ? smallButton : primaryButton} mt-auto`} disabled={selected} onClick={() => onAdd(product)} type="button">{selected ? 'Added' : 'Add product'}</button>
                </div>
              </article>
            );
          })}
...
function DraftPreview({ body, title, excerpt, featuredImage, products }: { body: string; title: string; excerpt: string; featuredImage: string; products: Product[] }) {
  ...
  return (
    <div className="max-h-[760px] overflow-y-auto rounded-2xl border border-white/10 bg-[#090d17] p-5 shadow-inner">
      ...
      <div className="cms-draft-preview mt-6 grid gap-5 text-base leading-8 text-slate-200 [&_a]:font-bold [&_a]:text-violet-300 [&_a]:underline [&_blockquote]:border-l-4 [&_blockquote]:border-violet-400 [&_blockquote]:bg-violet-500/10 [&_blockquote]:px-4 [&_blockquote]:py-2 [&_figcaption]:text-center [&_figcaption]:text-sm [&_figcaption]:text-slate-500 [&_h1]:text-3xl [&_h1]:font-black [&_h2]:text-2xl [&_h2]:font-black [&_h3]:text-xl [&_h3]:font-black [&_h4]:text-lg [&_h4]:font-black [&_img]:max-h-[520px] [&_img]:w-full [&_img]:object-contain [&_li]:ml-6 [&_ol]:list-decimal [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-black/50 [&_pre]:p-4 [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:border-white/10 [&_td]:p-2 [&_th]:border [&_th]:border-white/10 [&_th]:p-2 [&_ul]:list-disc">
